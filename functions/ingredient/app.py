...
def lambda_handler(event, context):
    ...
    try:
        ...
        # 라우터 키값에 따라 실행
        if routeKeyVal == "GET /ingredient":
            # id가 존재할때는 id에 해당하는 아이템만 검색 아니면 전체 검색
            print("============ GET All Ingredient ===============")
            sql = "select * from ingredient"
            curs.execute(sql)
            bodyData = curs.fetchall()

        elif routeKeyVal == "GET /ingredient/{id}":
            # 아이템 단항목정보
            idVal = event["pathParameters"]["id"]
            print("============ GET Single Ingredient with idVal ===============")
            print("GET Ingredient with idVal=" + idVal)
            sql = f"select * from ingredient where ing_id={idVal}"
            curs.execute(sql)
            bodyData = curs.fetchone()

        elif routeKeyVal == "POST /ingredient":
            ingredients = event["ingredients"]
            res = []
            for ingredient in ingredients:
                try:
                    sql = "INSERT INTO `ingredient` (`ing_name`) VALUES (%s);"
                    print(sql)
                    curs.execute(sql, ingredient)
                    res.append(f"{ingredient} is inserted into ingredient db")
