...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Path: GET /recipe/{id}/comment
        idVal = event["pathParameters"]["id"]

        statusCodeVal = 201
        print("============ GET Recipe Comments of Recipe ===============")
        # 댓글
        comment_query = f"select * from `recipe_comment` rc LEFT JOIN `user` u ON rc.user_uid = u.user_uid  where rc.recipe_id={idVal} "
