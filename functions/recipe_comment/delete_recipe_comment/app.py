...
def lambda_handler(event, context):
    ...
    try:
        ...
        rcmt_id = event["pathParameters"]["comment_id"]
        statusCodeVal = 201
        print("============ DELETE Recipe Comment ===============")
        requestJSON = json.loads(event["body"])
        requestJSON["user_uid"] = get_user_uid(curs, event["headers"]["authorization"])
        print(requestJSON)  # dict

        sql = f"DELETE FROM `recipe_comment` WHERE recipe_id={recipe_id} AND rcmt_id={rcmt_id};"
        print(sql)
        curs.execute(sql)
        conn.commit()
        bodyData = "comment deleted"
